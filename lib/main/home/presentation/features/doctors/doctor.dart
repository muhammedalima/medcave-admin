...
import 'package:medcave/common/service/firebase_doctor.dart';

class DoctorsTab extends StatelessWidget {
  final String searchQuery;
  final Function(String) logActivity;
  final FirebaseComponentService _firebaseService = FirebaseComponentService();

  DoctorsTab({required this.searchQuery, required this.logActivity, super.key});
                          ...
                          if (time != null) {
                            setState(() {
                              final String formattedTime = DateFormat('h:mm a')
                                  .format(DateTime(
                                      2022, 1, 1, time.hour, time.minute));
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  await FirebaseComponentService().addDoctor(
                  ...
                  logActivity(
                      'Added doctor: Dr. ${firstNameController.text.trim()} ${lastNameController.text.trim()}');
  ...
  void _showEditDialog(
    ...
    final specializationController =
        TextEditingController(text: data['specialization']);
    final experienceController =
        TextEditingController(text: data['experience']?.toString() ?? '');
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  logActivity(
                      'Updated doctor: Dr. ${firstNameController.text.trim()} ${lastNameController.text.trim()}');
                  Navigator.pop(context);
                },
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              await _firebaseService.deleteDoctor(docId);
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return _buildEmptyState(
              'No doctors available', 'Add a new doctor using the + button');
        }
        ...
        var filteredDocs = snapshot.data!.docs.where((doc) {
          ...
          return fullName.toLowerCase().contains(searchQuery.toLowerCase()) ||
              specialization.toLowerCase().contains(searchQuery.toLowerCase());
        }).toList();

        if (filteredDocs.isEmpty) {
          return _buildEmptyState(
              'No matching doctors', 'Try a different search term');
        }
                  ...
                  ExpansionTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Text(data['specialization'] ?? 'General Physician',
                            style: TextStyle(
                      ...
                      children: [
                        ...
                        IconButton(
                            ...
                            onPressed: () => _showDeleteDialog(
                                context, doc.id, "Dr. $fullName")),
                      ],
                    ),
                    children: [
                      if (data['timeslots'] != null &&
                          (data['timeslots'] as List).isNotEmpty)
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            ...
                            Wrap(
                              spacing: 8,
                              runSpacing: 8,
                              children: (data['timeslots'] as List)
                                  .map((slot) => Chip(
