...
import 'package:medcave/common/service/firebase_medicine.dart';

class MedicinesTab extends StatelessWidget {
  final String searchQuery;
  final Function(String) logActivity;
  final FirebaseMedicineService _medicineService =
      FirebaseMedicineService(); // Initialize the service
  ...
  static void showAddDialog(
    ...
    final FirebaseMedicineService _medicineService = FirebaseMedicineService();
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    ...
                    logActivity(
                        'Added medicine: ${nameController.text.trim()}');
              ...
              ElevatedButton(
                onPressed: () async {
                  ...
                  try {
                    ...
                    logActivity(
                        'Updated medicine: ${nameController.text.trim()}');
                    Navigator.pop(context);
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              try {
                await _medicineService.deleteMedicine(docId);
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      stream: _medicineService.getMedicinesStream(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: CircularProgressIndicator());
        }
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return _buildEmptyState('No medicines available',
              'Add a new medicine using the + button');
        }
        ...
        if (filteredDocs.isEmpty) {
          return _buildEmptyState(
              'No matching medicines', 'Try a different search term');
        }
              ...
              child: Column(
                children: [
                  ListTile(
                    ...
                    title: Text(data['name'] ?? 'Unnamed Medicine',
