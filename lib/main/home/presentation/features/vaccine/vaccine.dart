...
class VaccinesTab extends StatelessWidget {
  ...
  const VaccinesTab({required this.searchQuery, required this.logActivity, super.key});
          ...
          return AlertDialog(
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  TextField(controller: nameController, decoration: const InputDecoration(labelText: 'Vaccine Name*', border: OutlineInputBorder())),
                  const SizedBox(height: 16),
                  TextField(controller: descriptionController, decoration: const InputDecoration(labelText: 'Description', border: OutlineInputBorder(), hintText: 'Brief description of the vaccine'), maxLines: 3),
                  const SizedBox(height: 16),
                  TextField(controller: ageGroupController, decoration: const InputDecoration(labelText: 'Age Group', border: OutlineInputBorder(), hintText: 'E.g., 0-5 years, Adults, etc.')),
                  const SizedBox(height: 16),
              ...
              ElevatedButton(
                onPressed: () {
                  if (nameController.text.trim().isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please enter a vaccine name')));
                    return;
                  }
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  TextField(controller: ageGroupController, decoration: const InputDecoration(labelText: 'Age Group', border: OutlineInputBorder())),
                  const SizedBox(height: 16),
                  SwitchListTile(title: const Text('Available'), value: isAvailable, onChanged: (value) => setState(() => isAvailable = value)),
                ],
              ),
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  FirebaseFirestore.instance.collection('vaccines').doc(docId).update({
                    'name': nameController.text.trim(),
                    'description': descriptionController.text.trim(),
                    'ageGroup': ageGroupController.text.trim(),
                    'available': isAvailable,
                  ...
                  logActivity('Updated vaccine: ${nameController.text.trim()}');
                  Navigator.pop(context);
                },
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              FirebaseFirestore.instance.collection('vaccines').doc(docId).delete();
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return _buildEmptyState('No vaccines available', 'Add a new vaccine using the + button');
        }
        ...
        if (filteredDocs.isEmpty) {
          return _buildEmptyState('No matching vaccines', 'Try a different search term');
        }
              ...
              child: Column(
                children: [
                  ListTile(
                    ...
                    title: Text(data['name'] ?? 'Unnamed Vaccine', style: const TextStyle(fontWeight: FontWeight.bold)),
                    subtitle: Column(
